	...
	_allVehicles = vehicles;
	{
		...
		if(_check != 1) then 
		{
			...
			if(_legit != 1) then
			{
				...
				if((_vehicleType isKindOf "Helicopter") AND !(_vehicleType isKindOf "ParachuteBase")) then {
					deleteVehicle _x;
				};
				if((_vehicleType isKindOf "Plane") AND !(_vehicleType isKindOf "BIS_Steerable_Parachute")) then {
					deleteVehicle _x;
				};
